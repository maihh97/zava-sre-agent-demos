...
.DESCRIPTION
    ...
    4. Deploys the 3 web apps (main .NET app, wellbeing portal, warranty API)
...
.PARAMETER SqlLocation
    Azure SQL region. Default: eastus for restricted demo subscriptions.
...
.EXAMPLE
    .\deploy.ps1 -ResourceGroup rg-zava -Location westus2
...
if (-not $SkipInfra) {
    ...
    Write-Host "     Wellbeing URL:  $($outputs.itPortalUrl.value)"
...
if (-not $SkipSeed) {
    Write-Step "Database initialization"
    Write-Ok "The main app will initialize the private database on first startup"
} else {
...
if (-not $SkipApps) {
    ...
    # ── 3b. Wellbeing Portal (Node) ──────────────────────────
    Write-Step "Deploying wellbeing portal ($ItPortalName)..."
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Err "Wellbeing portal deployment failed"
        exit 1
    }
    Write-Ok "Wellbeing portal deployed: https://$ItPortalName.azurewebsites.net"
...
Write-Host "     • Agent 1: SQL & App Performance (monitor $ResourceGroup)" -ForegroundColor Gray
Write-Host "     • Agent 2: IT Support & ServiceNow (monitor $ResourceGroup)" -ForegroundColor Gray
Write-Host ""
Write-Host "  2. Add MCP connectors in the SRE Agent portal:" -ForegroundColor White
Write-Host "     • SQL MCP:    Configure with Microsoft Entra authentication" -ForegroundColor Gray
Write-Host "       Server:     $SqlServer.database.windows.net" -ForegroundColor DarkGray
...
Write-Host "    Wellbeing:     https://$ItPortalName.azurewebsites.net" -ForegroundColor Gray
