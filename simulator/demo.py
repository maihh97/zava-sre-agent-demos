...
APP_NAME       = os.environ.get("ZAVA_APP_NAME", "app-zava956235")
SQL_SERVER   = os.environ.get("ZAVA_SQL_SERVER",   "sql-zava956235.database.windows.net")
SQL_DATABASE = os.environ.get("ZAVA_SQL_DATABASE", "sqldb-zava956235")
SQL_USER     = os.environ.get("ZAVA_SQL_USER",     "<SQL_ADMIN_USER>")
SQL_PASSWORD = os.environ.get("ZAVA_SQL_PASSWORD", "<SQL_PASSWORD>")

APP_URL      = os.environ.get("ZAVA_APP_URL",      "https://app-zava956235.azurewebsites.net")
HEALTH_URL   = f"{APP_URL}/health"

SN_URL       = os.environ.get("ZAVA_SN_URL",  "https://<SN_INSTANCE>.service-now.com")
SN_USER      = os.environ.get("ZAVA_SN_USER", "admin")
SN_PASS      = os.environ.get("ZAVA_SN_PASS", "<SN_PASSWORD>")
...
def show_menu():
    ...
    tbl.add_row(
        "4", "🎫  ServiceNow Ticket",
        "Create a laptop replacement request.\n"
...
_AZ_CONN_CMD_GOOD = (
    f'az webapp config connection-string set '
    f'--name {APP_NAME} --resource-group {RESOURCE_GROUP} '
    '--connection-string-type SQLAzure '
    f'--settings "DefaultConnection=Server=tcp:{SQL_SERVER},1433;'
    f'Database={SQL_DATABASE};Authentication=Active Directory Managed Identity;'
...
_AZ_CONN_CMD_BAD = (
    ...
    f'--name {APP_NAME} --resource-group {RESOURCE_GROUP} '
    '--connection-string-type SQLAzure '
    f'--settings "DefaultConnection=Server=tcp:sql-zava956235-WRONG.database.windows.net,1433;'
    f'Database={SQL_DATABASE};Authentication=Active Directory Managed Identity;'
    'Encrypt=True;TrustServerCertificate=False;" '
    '-o none 2>&1'
)

_WEBHOOK_URL = os.environ.get("ZAVA_SRE_TRIGGER_URL", "<SRE_TRIGGER_URL>")
            ...
            while True:
                ...
                if key == "b" and not broken:
                    ...
                    if not seen_down:
                        ...
                        os.system(f"az webapp restart --name {APP_NAME} --resource-group {RESOURCE_GROUP} -o none 2>&1")
                        time.sleep(10)
...
def scenario_servicenow():
    ...
    def _create():
        ...
        payload = {
            "short_description":
                "Laptop replacement request - Alex Morgan",
            "description": (
                "Employee Alex Morgan (alex.morgan@company.com, EMP-10042, "
...
def _push_bad_config():
    ...
    bad_config = json.dumps({
        "ConnectionStrings": {"DefaultConnection": f"Server=tcp:sql-zava956235-WRONG.database.windows.net,1433;Database={SQL_DATABASE};Authentication=Active Directory Managed Identity;Encrypt=True;TrustServerCertificate=False;"},
...
def _restore_good_config():
    ...
    good_config = json.dumps({
        "ConnectionStrings": {"DefaultConnection": f"Server=tcp:{SQL_SERVER},1433;Database={SQL_DATABASE};Authentication=Active Directory Managed Identity;Encrypt=True;TrustServerCertificate=False;"},
        "ApplicationInsights": {"ConnectionString": ""},
        "Logging": {"LogLevel": {"Default": "Information", "Microsoft.AspNetCore": "Warning"}},
        "AllowedHosts": "*"
    }, indent=2)
